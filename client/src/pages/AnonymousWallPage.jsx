...
function DraggableCard({ message, constraintRef }) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
      className="absolute p-3 w-[280px] cursor-grab rounded-xl shadow-2xl backdrop-blur-md border border-white/10"
      style={{
        top: position.top,
        left: position.left,
    ...
    >
      {message.name && (
        <p className="font-semibold text-sm text-white">
          {message.name}{" "}
          <span className="text-orange-500 text-xs">• {message.role}</span>
        </p>
      )}
      <p className="mt-1 text-white text-base">{message.message}</p>
